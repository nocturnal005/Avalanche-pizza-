...
**Context.** ADR-007 descoped ordering because the ordering-flow screens did not exist and ADR-006 forbade inventing them. That reason has expired: the owner generated the designs himself — Basket (two variants), Checkout, Order Tracking, Track Your Order, My Account, and a Deal Builder — all archived under `design/stitch-exports/` on 2026-08-11. His checkout design specifies the delivery model and the payment rails ("Secured by Paystack"; MTN MoMo / Telecel Cash / AT Money chips). The Paystack analysis preserved in ADR-002 applies unchanged.

**Delivery area — Bechem only (Frank, 2026-08-11).** The checkout design offered three zones (Bechem Town GH₵ 10, Derma GH₵ 15, Techimantia GH₵ 20). The shop cannot service Derma or Techimantia yet, so **only Bechem Town ships**. Derma and Techimantia are retained in `src/content/delivery.ts` with `available: false` and their design fees marked unconfirmed; re-enabling either is a one-word change once the shop can deliver there. A build-time invariant fails if no zone is available. This matters more under ADR-009 than it did before: an unavailable zone reaching the dropdown would mean an order the kitchen cannot fulfil that the customer has **already paid for**.

**Consequence for the checkout UI.** With one zone, the designed dropdown has a single option. Options are (a) keep the select for fidelity and future zones, (b) render the zone and its fee as a fixed line. Deferred to the owner at the O1 review — and worth noting the delivery fee is now a constant GH₵ 10, so the basket could show a true total rather than the designed "Calculated at checkout", if he wants that.
